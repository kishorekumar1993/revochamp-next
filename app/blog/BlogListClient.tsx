...
const SearchBar = dynamic(() => import('@/components/blog/SearchBar'), { ssr: false });
...
export default function BlogListClient({
  ...
  // Track first mount to avoid duplicate fetch
  const isFirstMount = useRef(true);

  useEffect(() => {
    // Skip the first render – server already provided the data
    ...
    const fetchData = async () => {
      ...
      // Build URL based on category and page
      let url: string;
      ...
      try {
        ...
        let postsArray: BlogSummary[] = Array.isArray(data) ? data : data.data || [];
        ...
        setPosts(postsArray);
        // Update totalPages if the API returns it; otherwise keep existing
        if (data.totalPages) {
          setTotalPages(data.totalPages);
        } else {
          setTotalPages(1); // Fallback if not provided
        }
      } catch (err) {
