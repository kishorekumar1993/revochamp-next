...
export default function TutorialClient({
  ...
  const headings = useMemo(() => {
    return data.content
      .filter(
        (item): item is ContentItem & { type: "heading" } =>
          item.type === "heading",
      )
  ...
  const [quizStates, setQuizStates] =
    useState<QuizQuestionState[]>(initialQuizStates);
  ...
  useEffect(() => {
    ...
    if (saved.quizAnswers?.length === (data.quiz?.length || 0)) {
      setQuizStates((prev) =>
        prev.map((state, idx) => ({
          ...state,
          selectedAnswer: saved.quizAnswers[idx] ?? null,
        })),
      );
    }
      ...
      setTimeout(() => {
        if (scrollRef.current) {
          scrollRef.current.scrollTop =
            (scrollRef.current.scrollHeight * saved.scrollPercent) / 100;
        }
      }, 0);
    }
  }, [slug, data.quiz]);
  ...
  useEffect(() => {
    ...
    const handleScroll = throttle(() => {
      ...
      saveProgress(slug, {
        ...
        quizAnswers: quizStates.map((s) => s.selectedAnswer),
      });
    }, 100);
  ...
  const submitQuiz = () => {
    ...
    saveProgress(slug, {
      completed: true,
      quizAnswers: newStates.map((s) => s.selectedAnswer),
    });
  };
  ...
  const renderContent = () => {
    ...
    return data.content.map((item, idx) => {
      ...
      //   const headingId =
      //     headings[headingCounter]?.id || `heading-${headingCounter}`;
      //   headingCounter++;
      ...
      if (item.type === "heading") {
        ...
        "
            >
              <ReactMarkdown
                components={{
                  p: ({ children }) => <>{children}</>, // Unwrap default <p>
                }}
              >
                {item.value as string}
              </ReactMarkdown>
      ...
      if (item.type === "text") {
        ...
        return (
          <div
            key={key}
            className="my-6 prose prose-lg prose-slate 
                 prose-headings:font-semibold prose-headings:tracking-tight
                 prose-h1:text-4xl prose-h2:text-3xl prose-h3:text-2xl
                 prose-p:text-gray-700 prose-p:leading-relaxed
                 prose-a:text-blue-600 prose-a:no-underline hover:prose-a:underline
                 prose-blockquote:border-l-4 prose-blockquote:border-blue-300 prose-blockquote:bg-blue-50/50 prose-blockquote:py-1 prose-blockquote:px-4
                 prose-code:bg-gray-100 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-sm prose-code:font-mono
                 prose-pre:bg-gray-900 prose-pre:text-gray-100
                 prose-ul:my-4 prose-li:my-1
                 max-w-none
                 dark:prose-invert dark:prose-p:text-gray-300 dark:prose-a:text-blue-400 dark:prose-code:bg-gray-800 dark:prose-blockquote:bg-gray-800/30"
          >
            <ReactMarkdown
              components={{
                p: ({ children }) => (
                ...
                // Optional: enhance other elements if needed
              }}
            >
              {String(item.value)}
            </ReactMarkdown>
      ...
      //     <ul
      //       key={key}
      //       className="list-disc pl-6 my-6 space-y-2 text-gray-700 marker:text-gray-400"
      //     >
      //       {items.map((li, i) => (
      //         <li key={`${key}-li-${i}`}>
      //           <ReactMarkdown
  ...
  // Parse items from array or newline‑separated string
  if (Array.isArray(item.value)) {
    items = item.value;
  } else if (typeof item.value === "string") {
    const lines = item.value.split("\n");
    
    // Detect list type from the first non‑empty line
    const firstItem = lines.find(line => line.trim() !== "");
    if (firstItem) {
      // Ordered list: starts with number + dot (e.g., "1. ")
      isOrdered = /^\d+\.\s/.test(firstItem.trim());
    }
    
    // Strip markers and trim
    items = lines
  ...
  // Common classes shared between <ul> and <ol>
  const listClasses = `
    pl-6 sm:pl-8
  ...
  // Choose list type based on detection
  const ListComponent = isOrdered ? "ol" : "ul";
  const listTypeClass = isOrdered ? "list-decimal" : "list-disc";

  return React.createElement(
    ListComponent,
    {
      key,
      className: `${listTypeClass} ${listClasses}`,
    },
    items.map((li, i) => (
      <li key={`${key}-li-${i}`} className="pl-1">
        <ReactMarkdown
          components={{
            p: ({ children }) => <>{children}</>, // Avoid nested <p>
            a: ({ children, href }) => (
              <a
                href={href}
      ...
      if (item.type === "table") {
        ...
        const normalised = rows.map((row: any) =>
          Array.isArray(row)
            ? row
            : row.cells || row.columns || Object.values(row),
        );
        return (
          <div
            key={key}
            className="my-8 overflow-x-auto rounded-lg border border-gray-200"
          >
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {normalised.map(
                  (rowCells: any[], i: Key | null | undefined) => (
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="lg:hidden flex bg-gray-50 border border-gray-200 rounded-lg p-0.5">
              <button
                ...
                className="px-3 py-1 text-[10px] font-bold text-gray-600 hover:bg-white rounded-md shadow-none hover:shadow-sm"
              >
                INDEX
              </button>
              <button
                onClick={() => setMobileQuickLinksOpen(true)}
      ...
      <MobileDrawer
        isOpen={mobileQuickLinksOpen}
        onClose={() => setMobileQuickLinksOpen(false)}
        title="Related"
      >
        <QuickLinksContent
          category={category}
        ...
        >
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <header className="mb-10">
              ...
              {data.subtitle && (
                <p className="mt-3 text-lg text-gray-500">{data.subtitle}</p>
              )}
            </header>
              ...
              <section className="mt-14">
                ...
                {!quizSubmitted ? (
                  <div className="mt-6 flex gap-3">
                    <button
                      onClick={submitQuiz}
                      className="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition shadow-sm"
                    >
                      Submit Answers
                    </button>
                    <button
                      onClick={resetQuiz}
                      className="px-5 py-2.5 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-100 transition"
                    >
                      Reset
                    </button>
                  </div>
            ...
            <footer className="mt-16 pt-8 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row justify-between gap-4">
                {prevTopic ? (
                  <Link
                    href={`/tech/${category}/${prevTopic.slug}`}
                    className="group flex-1 p-4 border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-sm transition"
                  >
                ...
                {nextTopic && (
                  <Link
                    href={`/tech/${category}/${nextTopic.slug}`}
                    className="group flex-1 p-4 border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-sm transition"
                  >
...
function Breadcrumbs({ category, title }: { category: string; title: string }) {
  return (
    <nav className="hidden sm:flex items-center text-sm">
      ...
      <Link href="/tech" className="text-gray-500 hover:text-gray-900">
        Learn
      </Link>
      <span className="mx-2 text-gray-300">/</span>
      <Link
        ...
        <div className="relative group overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5 shadow-lg shadow-gray-200/50">
          ...
          <div className="relative z-10">
            ...
            <p className="text-[11px] text-gray-400 leading-relaxed mb-4">
              Explore our comprehensive docs or start a chat with our tech
              experts.
            </p>
